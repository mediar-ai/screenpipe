...
export const ShareLogsButton = () => {
  ...
  const captureLastFiveMinutes = async () => {
    ...
    try {
      ...
      // Merge frames
      const mergeResponse = await fetch('http://localhost:3030/experimental/frames/merge', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const sendLogs = async () => {
    ...
    //   const BASE_URL = 'https://screenpi.pe'
      ...
      const {
        data: { signedUrl, path, signedUrlScreenshot, signedUrlVideo, screenshotPath, videoPath }
      ...
      const { data: { id } } = await confirmRes.json()
              ...
              <label className='cursor-pointer flex-none'>
                ...
                <Button
                  ...
                  className={`gap-2 h-9 px-4 rounded-full transition-all ${
                    screenshot ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/30 hover:bg-emerald-500/20' : ''
                  }`}
                  disabled={!!screenshot}
                  asChild
                >
              ...
              <Tooltip delayDuration={200}>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    className={`gap-2 h-9 px-4 rounded-full transition-all ${
                      mergedVideoPath ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/30 hover:bg-emerald-500/20' : ''
                    }`}
                  ...
                  >
                    ...
                    <span>recording</span>
                    <span className='ml-1 text-xs text-muted-foreground/70'>5m</span>
                  </Button>
                </TooltipTrigger>
                <TooltipContent 
                  side='bottom' 
