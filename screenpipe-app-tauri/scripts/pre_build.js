...
const config = {
  ...
  windows: {
    ...
    clblastUrl:
      "https://github.com/CNugteren/CLBlast/releases/download/1.6.2/CLBlast-1.6.2-windows-x64.zip",
...
async function isDenoInstalled() {
  // assuming it's installed in PATH
  try {
...
async function installDeno() {
  ...
  if (platform === "windows") {
    ...
    try {
      ...
      console.error(
        "failed to install/upgrade deno using chocolatey:",
        chocoError
      );
...
async function copyDenoBinary() {
  ...
  if (platform === "windows") {
    ...
    const chocoPathTools =
      "C:\\ProgramData\\chocolatey\\lib\\deno\\tools\\deno.exe";
  ...
  try {
    ...
    if (error.code === "ENOENT") {
      ...
      try {
        ...
        console.log(
          `deno binary copied successfully from PATH to ${denoDest1}`
        );
        ...
        console.log(
          "please ensure deno is installed and accessible in your PATH"
        );
      }
    } else if (error.code === "EPERM") {
      console.error(
        `permission denied when copying deno binary. trying elevated copy...`
      );
      await elevatedCopy(denoSrc, denoDest1);
      console.log(
        `deno binary copied successfully to ${denoDest1} using elevated permissions`
      );
    } else {
...
if (platform == "windows") {
  ...
  if (!(await fs.exists(config.openblasRealname)) && hasFeature("openblas")) {
    ...
    fs.cp(
      path.join(config.openblasRealname, "include"),
      path.join(config.openblasRealname, "lib"),
      { recursive: true, force: true }
    );
    // It tries to link only openblas.lib but our is libopenblas.lib`
    fs.cp(
      path.join(config.openblasRealname, "lib/libopenblas.lib"),
      path.join(config.openblasRealname, "lib/openblas.lib")
    );
  }
  ...
  if (platform === "windows") {
    const windowsConfig = {
      bundle: {
        ...
        externalBin: ["screenpipe"],
      },
    };
    await fs.writeFile(
      "tauri.windows.conf.json",
      JSON.stringify(windowsConfig, null, 4)
    );
  }
  if (platform === "linux") {
    // Add cuda toolkit depends package
    const tauriConfigContent = await fs.readFile("tauri.linux.conf.json", {
      encoding: "utf-8",
    });
...
if (hasFeature("opencl")) {
  if (platform === "windows") {
    const tauriConfigContent = await fs.readFile("tauri.windows.conf.json", {
      encoding: "utf-8",
    });
    const tauriConfig = JSON.parse(tauriConfigContent);
