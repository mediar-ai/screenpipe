...
END;
CREATE TRIGGER ocr_text_au AFTER UPDATE OF text ON ocr_text BEGIN
  UPDATE ocr_text SET text_length = LENGTH(NEW.text) WHERE id = NEW.id;
END;
...
END;
CREATE TRIGGER audio_transcriptions_au AFTER UPDATE OF transcription ON audio_transcriptions BEGIN
  UPDATE audio_transcriptions SET text_length = LENGTH(NEW.transcription) WHERE id = NEW.id;
END;
...
END;
CREATE TRIGGER ui_monitoring_au AFTER UPDATE OF text_output ON ui_monitoring BEGIN
  UPDATE ui_monitoring SET text_length = LENGTH(NEW.text_output) WHERE id = NEW.id;
END;
