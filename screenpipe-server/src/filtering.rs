...
fn keep_least_similar(chunks: &[String], percentage: f64) -> Vec<usize> {
    ...
    for (doc_id, chunk) in chunks.iter().enumerate() {
        let words: Vec<_> = chunk.split_whitespace()
            .map(|word| en_stemmer.stem(word).to_string())
            .collect();

        for word in words.iter() {
            *term_freq.entry((doc_id, word.clone())).or_insert(0) += 1;
            doc_freq.entry(word.clone()).or_insert_with(HashSet::new).insert(doc_id);
        }
    }
...
pub async fn filter_texts(timestamp: &str, memory_source: &str, pool: &SqlitePool) -> Result<String, Box<dyn Error + Send + Sync>> {
    ...
    debug!("Kept: {} texts, {} words", final_text_count, final_word_count);

    let output = kept_texts.iter().map(|s| s.as_str()).collect::<Vec<&str>>().join("\n");
