    ...
    loop {
        ...
        match run_ui_monitoring(&script_path).await {
            ...
            Err(e) => {
                ...
                warn!("restarting ui monitoring in 5 seconds...");
        ...
        select! {
            line = stdout_reader.next_line() => {
                match line {
                    ...
                    Ok(None) => {
                        debug!("ui monitoring: stdout closed");
                        break;
                    },
                    Err(e) => {
                        error!("ui monitoring: error reading stdout: {}", e);
                        break;
                    }
                }
            }
            line = stderr_reader.next_line() => {
                match line {
                    ...
                    Ok(None) => {
                        debug!("ui monitoring: stderr closed");
                        break;
                    },
                    Err(e) => {
                        error!("ui monitoring: error reading stderr: {}", e);
                        break;
                    }
                }
            }
            status = child.wait() => {
                match status {
                    Ok(status) => {
                        if !status.success() {
                            return Err(anyhow::anyhow!("ui monitoring crashed with status: {}", status));
                        }
                        return Ok(());
                    }
                    Err(e) => {
                        return Err(anyhow::anyhow!("ui monitoring wait error: {}", e));
                    }
                }
            }
        }
