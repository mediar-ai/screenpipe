...
exports.TimelineSearch2 = TimelineSearch2;
...
function extractKeywords(frames) {
    ...
    const patterns = {
        ...
        appName: /\b(?:vscode|chrome|firefox|safari|slack|discord|zoom|teams|figma|notion|github)\b/gi,
        codeTerms: /\b(?:function|class|const|let|var|import|export|async|await|return|if|else|for|while)\b/g,
    ...
    const scoredKeywords = Array.from(keywords.entries()).map(([word, count]) => {
        ...
        if (word.match(patterns.email))
            score *= 2;
        if (word.match(patterns.properNoun))
            score *= 1.5;
        if (word.match(patterns.date))
            score *= 1.3;
        if (word.match(patterns.appName))
            score *= 1.2;
        // Penalize very common words
        if (count > frames.length / 2)
            score *= 0.5;
        return { word, score };
    });
...
function TimelineSearch2({ frames, onResultSelect, onSearchResults, }) {
    ...
    const handleSearch = (0, react_1.useCallback)((query) => {
        ...
        const results = frames.reduce((matches, frame, index) => {
        ...
        if (results.length > 0) {
            onSearchResults === null || onSearchResults === void 0 ? void 0 : onSearchResults(results);
            setOpen(false);
        }
        setIsSearching(false);
    }, [frames, onSearchResults]);
    return (<>
      <button_1.Button variant="ghost" size="icon" onClick={() => setOpen(true)} className="rounded-full">
        <lucide_react_1.Search className="h-4 w-4"/>
      </button_1.Button>

      <dialog_1.Dialog open={open} onOpenChange={setOpen}>
        <dialog_1.DialogPortal>
          <dialog_1.DialogOverlay className="backdrop-blur-sm"/>
          <dialog_1.DialogContent className="sm:max-w-[500px]">
            ...
            if (e.key === "Enter" && !isSearching) {
                ...
                {isSearching && (<lucide_react_1.Loader2 className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 animate-spin"/>)}
              </div>

              <framer_motion_1.motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-6">
                <p className="text-sm text-muted-foreground mb-3">
                  common keywords
                </p>
                <div className="flex flex-wrap gap-2">
                  {keywords.map((keyword) => (<button_1.Button key={keyword} variant="outline" size="sm" onClick={() => handleSearch(keyword)} className="text-xs">
                      {keyword}
                    </button_1.Button>))}
                </div>
