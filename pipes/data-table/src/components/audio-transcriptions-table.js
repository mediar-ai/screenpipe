...
exports.AudioTranscriptionsTable = AudioTranscriptionsTable;
...
const columns = [
    ...
    },
    {
        ...
        header: "Device",
        cell: ({ row }) => {
            const deviceName = `${row.getValue("device")}${row.original.is_input_device ? " (input)" : " (output)"}`;
            return (<div className="relative group">
          <badge_1.Badge variant="outline" className="max-w-[200px] truncate">
            {deviceName}
          </badge_1.Badge>
    ...
    },
    {
        ...
        {(_a = row.getValue("speaker_id")) !== null && _a !== void 0 ? _a : "unknown"}
      </badge_1.Badge>);
        },
    },
    {
        ...
        {(_a = row.getValue("speaker_name")) !== null && _a !== void 0 ? _a : "unnamed"}
      </badge_1.Badge>);
        },
    },
    {
        ...
        header: "Engine",
        cell: ({ row }) => <badge_1.Badge>{row.getValue("transcription_engine")}</badge_1.Badge>,
    },
    {
        id: "actions",
        cell: ({ row }) => {
          ...
          <dropdown_menu_1.DropdownMenuContent align="end">
            ...
            <dropdown_menu_1.DropdownMenuItem onClick={() => navigator.clipboard.writeText(transcription.transcription)}>
              copy text
            </dropdown_menu_1.DropdownMenuItem>
            <dropdown_menu_1.DropdownMenuItem onClick={() => navigator.clipboard.writeText(JSON.stringify(transcription, null, 2))}>
              copy raw data
            </dropdown_menu_1.DropdownMenuItem>
...
function AudioTranscriptionsTable() {
    var _a;
    ...
    const fetchData = () => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
          ...
          `,
            ...
            setError(`failed to load audio transcriptions: ${error instanceof Error ? error.message : "unknown error"}`);
        }
        finally {
            setIsLoading(false);
        }
    ...
    const loadDevices = () => __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            setAvailableAudioDevices(audioDevices.map((device) => ({
                name: device.name
                    .replace("(input)", "")
                    .replace("(default)", "")
                    .trimEnd(),
    ...
    const loadSpeakers = () => __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const speakers = yield response.json();
      ...
      <div className="flex items-center justify-between py-4">
        <div className="flex items-center space-x-4">
          <div className="relative">
            ...
            <input_1.Input placeholder="filter transcription..." value={transcriptionFilter} onChange={(event) => {
            setTranscriptionFilter(event.target.value);
            setPageIndex(0);
        }} className="max-w-sm pl-8"/>
          </div>

          {availableAudioDevices.length > 0 && (<select_1.Select value={deviceFilter} onValueChange={(value) => {
                setDeviceFilter(value);
                setPageIndex(0);
            }}>
              <select_1.SelectTrigger className="w-[200px]">
                <lucide_react_1.Mic2 className="mr-2 h-4 w-4"/>
                <select_1.SelectValue placeholder="filter by device..."/>
              </select_1.SelectTrigger>
              <select_1.SelectContent>
                <select_1.SelectGroup>
                  <select_1.SelectLabel>audio devices</select_1.SelectLabel>
                  {availableAudioDevices.map((device) => (<select_1.SelectItem key={device.name} value={device.name}>
          ...
          {availableSpeakers.length > 0 && (<select_1.Select value={speakerFilter} onValueChange={(value) => {
                setSpeakerFilter(value);
                setPageIndex(0);
            }}>
              <select_1.SelectTrigger className="w-[200px]">
                <lucide_react_1.Users className="mr-2 h-4 w-4"/>
                <select_1.SelectValue placeholder="filter by speaker..."/>
              </select_1.SelectTrigger>
              <select_1.SelectContent>
                <select_1.SelectGroup>
                  <select_1.SelectLabel>speakers</select_1.SelectLabel>
                  {availableSpeakers.map((speaker) => (<select_1.SelectItem key={speaker.id} value={speaker.name}>
                      {speaker.name}
                    </select_1.SelectItem>))}
                </select_1.SelectGroup>
              </select_1.SelectContent>
