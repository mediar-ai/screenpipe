...
const Pipe: React.FC = () => {
  ...
  const getMaxLimit = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3030/search?content_type=all&limit=30&offset=0&start_time=${startTimeStr}&end_time=${endTimeStr}&min_length=50&max_length=10000`
      );
      if (!response.ok) {
  ...
  const fetchVideoContent = async (startTime: string, endTime: string) => {
    try {
      pipe.captureMainFeatureEvent("pipe-for-loom", {
        action: "fetch-video-content",
      });
      const limit = await getMaxLimit();
      console.log("Limit:", limit);
      const response = await fetch(
        `http://localhost:3030/search?q=&limit=${limit}&offset=0&content_type=ocr&start_time=${startTime}&end_time=${endTime}&min_length=50&max_length=200`
      );
      if (!response.ok) {
        throw new Error(`http error! status: ${response.status}`);
      }
  ...
  const fetchAudioContent = async (startTime: string, endTime: string) => {
    try {
      const limit = await getMaxLimit();
      const response = await fetch(
        `http://localhost:3030/search?q=&limit=${limit}&offset=0&content_type=audio&start_time=${startTime}&end_time=${endTime}&min_length=50&max_length=200`
      );
      if (!response.ok) {
        throw new Error(`http error! status: ${response.status}`);
      }
  ...
  const handleVideoMerging = async () => {
    try {
      ...
      const videoPaths = (await fetchVideoContent(
        startTimeStr,
        endTimeStr
      )) as string[];
      ...
      if (videoPaths.length < 2) {
        toast({
          ...
          variant: "destructive",
          description:
            "insufficient video contents in that time period, please try again later!",
          duration: 3000,
        });
        setIsMerging(false);
        return;
  ...
  const handleAudioMerging = async () => {
    try {
      ...
      const audioPaths = (await fetchAudioContent(
        startTimeStr,
        endTimeStr
      )) as string[];
