...
exports.MeetingPrepDetails = MeetingPrepDetails;
function MeetingPrepDetails({ aiPrep }) {
    return (<div className="space-y-2 text-sm border-t border-border pt-2">
      {/* Previous Context */}
      <div>
      ...
      {/* Person Context */}
      {Object.entries(aiPrep.previousContext.personContext).map(([person, context]) => (<div key={person} className="space-y-1">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div>
              ...
              <ul className="list-disc list-inside ml-2">
                {context.pastDecisions.map((decision, i) => (<li key={i}>{decision}</li>))}
              </ul>
            </div>
            <div>
              <span className="text-muted-foreground font-semibold">agreed next steps:</span>
              <ul className="list-disc list-inside ml-2">
                {aiPrep.previousContext.agreedNextSteps.map((step, i) => (<li key={i}>{step}</li>))}
              </ul>
            </div>
          </div>
        </div>))}
      ...
      <div className="grid grid-cols-3 gap-4 text-xs">
        <div>
          ...
          <ul className="list-disc list-inside ml-2">
            {aiPrep.suggestedPrep.reviewPoints.map((point, i) => (<li key={i}>{point}</li>))}
          </ul>
        </div>
        <div>
          <div className="font-semibold text-muted-foreground mb-1">discussion topics</div>
          <ul className="list-disc list-inside ml-2">
            {aiPrep.suggestedPrep.discussionTopics.map((topic, i) => (<li key={i}>{topic}</li>))}
          </ul>
        </div>
        <div>
          <div className="font-semibold text-muted-foreground mb-1">meeting tips</div>
          <ul className="list-disc list-inside ml-2">
            {aiPrep.suggestedPrep.meetingTips.map((tip, i) => (<li key={i}>{tip}</li>))}
          </ul>
        </div>
      </div>
    </div>);
