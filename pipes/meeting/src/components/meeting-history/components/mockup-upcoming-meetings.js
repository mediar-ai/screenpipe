...
const meeting_card_1 = require("./meeting-card");
...
function UpcomingMeetings() {
    ...
    const getUpcomingTime = (meetingStart) => {
    ...
    return (<div className="space-y-2">
      {MOCK_UPCOMING_MEETINGS.map((meeting) => {
            var _a, _b;
            return (<div key={meeting.id} className="relative">
          <div className="absolute -left-0 z-10 -top-2">
            <div className="flex items-center gap-1 text-xs text-muted-foreground bg-accent rounded px-1 border border-border/40">
              {((_a = meeting.humanName) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes("recording demo")) && (<div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"/>)}
              {((_b = meeting.humanName) === null || _b === void 0 ? void 0 : _b.toLowerCase().includes("recording demo")) ?
                    "now" :
                    `${getUpcomingTime(meeting.meetingStart)[0]} ${getUpcomingTime(meeting.meetingStart)[1]}`}
            </div>
          </div>
          <meeting_card_1.MeetingCard meeting={meeting} onUpdate={handleUpdate} settings={settings}/>
        </div>);
        })}
    </div>);
}
