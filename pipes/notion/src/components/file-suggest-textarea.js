...
function FileSuggestTextarea({ value, setValue, className = "", placeholder, disabled = false, }) {
    const { settings } = (0, use_pipe_settings_1.useNotionSettings)();
    ...
    const debouncedFetch = (0, react_1.useCallback)((search) => {
        ...
        searchTimeout.current = setTimeout(() => __awaiter(this, void 0, void 0, function* () {
            var _a, _b;
            try {
                const res = yield fetch(`/api/notion/pages?q=${encodeURIComponent(search)}&accessToken=${(_a = settings === null || settings === void 0 ? void 0 : settings.notion) === null || _a === void 0 ? void 0 : _a.accessToken}`);
                const data = yield res.json();
                setFiles((_b = data.pages) !== null && _b !== void 0 ? _b : []);
            }
            catch (err) {
    ...
    return (<div className="relative">
      ...
      {showSuggestions && !disabled && (<div className="fixed z-50 w-[400px] max-h-48 overflow-y-auto bg-background border rounded-md shadow-lg" style={{
          ...
          <cmdk_1.Command>
            <cmdk_1.Command.List>
              {files.map((file) => {
                return (<cmdk_1.Command.Item key={file.id} onSelect={() => handleSuggestionSelect(file)} className="px-2 py-1 hover:bg-accent cursor-pointer flex flex-col gap-0.5">
