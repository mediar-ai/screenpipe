...
- **url filtering**: to filter by URL (for browser content), use the window_name parameter with the URL pattern
  ```bash
  # Filter content from a specific website
  curl "http://localhost:3030/search?content_type=ocr&window_name=github.com"
  ```
...
```bash
# Find "meeting" mentions in Chrome from a specific date range
curl "http://localhost:3030/search?q=meeting&content_type=ocr&app_name=chrome&start_time=2024-05-01T00:00:00Z&end_time=2024-05-02T23:59:59Z"
```
