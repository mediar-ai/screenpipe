...
- **realtime-audio-transcription-engine** (`--realtime-audio-transcription-engine <ENGINE>`): realtime engine
  - same options as audio-transcription-engine
...
- **enable-realtime-audio-transcription** (`--enable-realtime-audio-transcription`): enable realtime transcription
  ...
  - uses: `--realtime-audio-transcription-engine` (default: whisper-tiny)
...
indexing allows you to add external screen recordings to screenpipe, for example it could be your iphone screen recordings, your physical journal photos, etc.

companies use this to index their product's screen recordings, for example.

```bash
# index video files
screenpipe index <PATH> [--data-dir <DIR>] [--output <FORMAT>] [--pattern <REGEX>] [--ocr-engine <ENGINE>] [--metadata-override <PATH>]
```
