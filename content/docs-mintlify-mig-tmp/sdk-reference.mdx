...
the operator api allows for more powerful desktop automation using accessibility roles and elements. it provides a robust way to interact with applications programmatically.

OPERATOR ONLY WORKS ON MACOS FOR NOW - screenpipe backend need to be running also

on Windows/Linux, please use the [input control api](#input-control-api) instead.
...
#### using operator api with vercel ai sdk
