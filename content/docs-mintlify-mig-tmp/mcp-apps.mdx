...
```html
<!-- screenpipe-integrations/screenpipe-mcp/ui/my-app.html -->
<!DOCTYPE html>
<html>
<head>
  <style>
...
<body>
  ...
  <script>
    ...
    function search() {
      ...
      app.callTool('search-content', { q: query, limit: 10 });
    }
...
```
screenpipe-integrations/screenpipe-mcp/
├── src/
│   └── index.ts          # MCP server
├── ui/
...
```bash
cd screenpipe-integrations/screenpipe-mcp
npm run build
...
- [screenpipe MCP source](https://github.com/mediar-ai/screenpipe/tree/main/screenpipe-integrations/screenpipe-mcp)
