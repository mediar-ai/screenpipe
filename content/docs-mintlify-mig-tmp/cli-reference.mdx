...
{/* ENVIRONMENT VARIABLES WILL AUTOMATICALLY POPULATE HERE */}
...
- **`SAVE_RESOURCE_USAGE`** (optional, defaults to **`false`**)
  When set to **`true`**, resource usage will be logged to a timestamped JSON file in your home directory for performance monitoring.

---

#### Deepgram & Other Engines Settings

- **`DEEPGRAM_API_KEY`** (required if Deepgram engine is selected)
  Authenticates requests to the Deepgram API.

- **`CUSTOM_DEEPGRAM_API_TOKEN`** (required if Deepgram engine is selected for real-time audio transcription)
  Authenticates requests to the Deepgram API for real-time audio transcription.
...
- **`UNSTRUCTURED_API_KEY`** (required if Unstructured engine is selected)
  Authenticates requests to the Unstructured API.

---

#### Other Settings

- **`TAURI_ENV_DEBUG`** (optional, defaults to **`false`**)
  Enables debug mode for Tauri, which may affect performance and logging.
