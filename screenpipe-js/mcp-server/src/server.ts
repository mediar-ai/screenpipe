...
console.log(
  "browser pipe operator methods:",
  Object.keys(browserPipe.operator)
);
console.log(
  "browser pipe implementation type:",
  typeof browserPipe.operator.get_text
);
...
server.tool(
  ...
  async (input: OpenApplicationInput) => {
    ...
    try {
      const success = await browserPipe.operator.openApplication(
...
server.tool(
  ...
  async (input: OpenUrlInput) => {
    ...
    try {
      const success = await browserPipe.operator.openUrl(
        input.url,
        input.browser
      );
