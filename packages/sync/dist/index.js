...
function parseArgs() {
  ...
  for (let i2 = 0;i2 < args.length; i2++) {
    ...
    switch (arg) {
      case "--output":
      case "-o":
        config.outputDir = args[++i2];
        break;
      case "--hours":
      case "-h":
        config.hours = parseInt(args[++i2]) || 12;
        break;
      case "--git":
      case "-g":
        config.gitPush = true;
...
function processResults(results) {
  ...
  for (const r2 of results) {
    if (r2.type !== "OCR")
      continue;
    const text = r2.content.text?.trim();
    ...
    if (seen.has(hash))
      continue;
    seen.add(hash);
    const app = r2.content.app_name || "Unknown";
    byApp[app] = byApp[app] || [];
    byApp[app].push(text);
    timeline.push({
      time: r2.content.timestamp,
      app,
...
`).slice(0, 15000);
  ...
  if (config.anthropicKey) {
    if (config.verbose)
      console.error(`[ai] Using Claude (${condensed.length} chars)`);
    try {
      const client = new sdk_default({ apiKey: config.anthropicKey });
      ...
      console.error(`[error] Claude failed: ${e2}`);
    }
  }
  if (config.openaiKey) {
    if (config.verbose)
      console.error(`[ai] Using OpenAI`);
    try {
      ...
      console.error(`[error] OpenAI failed: ${e2}`);
    }
  }
  try {
    if (config.verbose)
      console.error(`[ai] Trying Ollama at ${config.ollamaUrl}`);
    ...
    if (config.verbose)
      console.error(`[warn] Ollama not available`);
  }
...
function formatMarkdown(summary) {
  ...
  const appList = Object.entries(apps).sort((a2, b2) => b2[1] - a2[1]).slice(0, 10).map(([app, mins]) => `- **${app}**: ~${mins} min`).join(`
`);
  const formatList = (items) => items.length > 0 ? items.map((i2) => `- ${i2}`).join(`
`) : "_None extracted_";
...
async function writeOutput(content, config, filename) {
  ...
  if (config.remote) {
    ...
    try {
      ...
      console.error(`[error] Remote sync failed: ${e2}`);
    }
  }
}
