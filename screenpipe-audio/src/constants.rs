...
pub static AUDIO_CONFIG: Lazy<AudioConfig> = Lazy::new(|| AudioConfig::from_env());
...
impl AudioConfig {
    pub fn from_env() -> Self {
        let chunk_duration_ms = env::var("AUDIO_CHUNK_DURATION_MS")
            .ok()
        ...
        Self {
            ...
            frame_size: env::var("AUDIO_FRAME_SIZE")
                ...
                .and_then(|v| v.parse().ok())
                .unwrap_or(1600),
            whisper_sample_rate: env::var("AUDIO_WHISPER_SAMPLE_RATE")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(16000), // Default Whisper sample rate
            chunk_duration_ms,
        }
    }
}
...
pub const OVERLAP_SAMPLES: usize = OVERLAP_SECONDS * 16000; // 16kHz sample rate
