...
use screenpipe_audio::audio_processing::{NoiseFilter, NoiseReductionModel};
...
async fn test_transcription_accuracy() {
    ...
    info!("starting transcription accuracy test");
    ...
    info!("creating noise filter");
    let noise_filter = Arc::new(Mutex::new(
        NoiseFilter::new(
            PathBuf::from(
                // TODO ADD PATH TO MODEL
                "<MODEL_PATH>",
            ),
            44100,
        )
    ...
    for (audio_file, expected_transcription) in test_cases {
        ...
        let task = tokio::spawn(async move {
            ...
            println!("resampling");
            let audio_data = resample(&audio_input.data, 44100, 16000).unwrap();
            println!("processing noise filter");

            let audio_data = noise_filter.lock().await.process(&audio_data).unwrap();
            println!("noise filter processed");

            // write to file as wav... this can be done with ffmpeg
            ...
            .unwrap();
            let mut writer = wav_writer.get_i16_writer(audio_data.len() as u32);
            ...
            let mut segments = prepare_segments(
                ...
                embedding_manager,
                embedding_extractor,
                audio_input.device.to_string().as_str(),
            )
            .await
            .unwrap();
