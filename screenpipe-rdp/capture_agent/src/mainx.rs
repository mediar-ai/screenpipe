...
use windows::Win32::System::RemoteDesktop::WTSVirtualChannelOpenEx;
use windows::Win32::System::RemoteDesktop::*;
use windows::core::Error;
use windows::core::{PCSTR, PWSTR};

fn capture_remote_screen(_server_handle: HANDLE, session_id: u32) {
    unsafe {
        // First check if the session is active
        let _session_state: u32 = 0;
        let _bytes_returned: u32 = 0;
        ...
        if channel.is_err() {
            ...
            if channel.is_err() {
                eprintln!(
                    "Failed to open RDPDISPLAY channel: {:?}",
                    Error::from_thread()
                );
                return;
            }
        ...
        .is_ok()
        {
            ...
            // Create image buffer
            let img_buffer =
                ImageBuffer::<Rgba<u8>, _>::from_raw(width, height, rgba_data).unwrap();
        ...
        let _ = WTSVirtualChannelClose(channel.unwrap());
    }
}
...
fn main() -> windows::core::Result<()> {
    ...
    unsafe {
        ...
        let server_handle = {
            let server_url = env::var("RDP_SERVER_URL").unwrap_or_else(|_| {
            ...
            WTSEnumerateSessionsW(Some(server_handle), 0, 1, &mut sessions, &mut session_count)?;
        ...
        for session_info in sessions_slice {
            ...
            let _buffer_size: u32 = 0;
            let _buffer_ptr: *mut PWSTR = std::ptr::null_mut();
            ...
            .is_ok()
            {
                let winstation: String = if !winstation_ptr.is_null() {
                ...
                println!(
                    "Session {} winstation: {}",
                    session_info.SessionId, winstation
                );

                // Check if it's an RDP session
                if winstation.contains("RDP")
                    || winstation.contains("Tcp")
                    || session_info.State.0 < 7
                {
