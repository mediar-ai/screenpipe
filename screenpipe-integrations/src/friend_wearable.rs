...
pub async fn initialize_friend_wearable_loop<DB: FriendWearableDatabase + Send + Sync + 'static>(uid: String, db: Arc<DB>) {
    tokio::spawn(async move {
        ...
        debug!("Friend_wearable: loop started with interval: {:?}", interval);
...
async fn filter_and_send_data<DB: FriendWearableDatabase + Send + Sync>(
    ...
    for source in &["screen", "audio"] {
        let (texts, min_chunk_id, max_chunk_id, min_timestamp, max_timestamp) = 
            db.get_chunked_data_since_timestamp(source, uid, since).await?;
        ...
        if !texts.is_empty() {
            ...
            debug!("Friend_wearable: Created payload with request_id: {}", request_id);

            send_data_to_friend_wearable_with_payload(payload, uid).await?;
            db.insert_friend_wearable_request(&request_id.to_string(), source, &chunk_id_range, &format!("{}-{}", min_timestamp.timestamp(), max_timestamp.timestamp()), uid).await?;
...
async fn send_data_to_friend_wearable_with_payload(payload: serde_json::Value, uid: &str) -> Result<(), Box<dyn StdError + Send + Sync>> {
    ...
    if status.is_success() {
        ...
        error!("Friend_wearable: Unexpected response from friend wearable API: {}", status);
