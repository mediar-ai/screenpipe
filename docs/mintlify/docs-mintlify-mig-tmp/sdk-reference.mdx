---
title: "screenpipe JavaScript SDK - query screen data from TypeScript"
sidebarTitle: "sdk reference"
description: "Use the screenpipe JavaScript SDK to query screen recordings, OCR text, and audio transcriptions from your TypeScript or Node.js applications."
icon: "js"
---
...
const results = await pipe.queryScreenpipe({
  ...
  minLength: 10,
  maxLength: 1000,
  speakerIds: [1, 2],        // filter audio by speaker IDs
  speakerName: "John",       // filter audio by speaker name (partial match)
  frameName: "screenshot.png",
  browserUrl: "github.com/screenpipe/screenpipe" // filter by browser URL
})
```

### ui events api (accessibility data - macOS)

screenpipe captures UI events via accessibility APIs on macOS. this is the third data modality alongside vision (OCR) and audio.

```typescript
// search UI events via REST API
const response = await fetch('http://localhost:3030/ui-events?' + new URLSearchParams({
  event_type: 'text',      // click, text, scroll, key, app_switch, window_focus, clipboard
  app_name: 'Slack',       // filter by app
  limit: '50',
  start_time: '2024-03-10T12:00:00Z',
  end_time: '2024-03-10T13:00:00Z',
}));
...
// - text_content: for text/clipboard events, what was typed/copied
// - x, y: for click events, coordinates
// - element: accessibility element info (label, role) for click events
```

**event types:**
- `text` - aggregated keyboard input (what was typed)
- `click` - mouse clicks with element accessibility labels  
- `app_switch` / `window_focus` - app and window changes
- `clipboard` - copy/paste operations with content
- `scroll` - scroll events with delta values

```typescript
// get UI event statistics (app usage analysis)
const statsResponse = await fetch('http://localhost:3030/ui-events/stats?' + new URLSearchParams({
  start_time: '2024-03-10T00:00:00Z',
  end_time: '2024-03-10T23:59:59Z',
}));
const stats = await statsResponse.json();
// returns: [{ app_name, event_type, count }, ...]
```
...
check how the obsidian pipe implements this, [route](https://github.com/screenpipe/screenpipe/blob/main/pipes/obsidian/src/app/api/log/route.ts) and [pipe.json](https://github.com/screenpipe/screenpipe/blob/main/pipes/obsidian/pipe.json) for a complete example.
...
see the [obsidian pipe](https://github.com/screenpipe/screenpipe/tree/main/pipes/obsidian) for a complete example of settings management.
...
see the [obsidian pipe](https://github.com/screenpipe/screenpipe/tree/main/pipes/obsidian) for the complete implementation.
...
check out our [production pipe examples](https://github.com/screenpipe/screenpipe/tree/main/pipes) on github to see more ai integration patterns.
...
- https://github.com/screenpipe/screenpipe/blob/main/packages/screenpipe-js/browser-sdk/src/index.ts
- https://github.com/screenpipe/screenpipe/blob/main/packages/screenpipe-js/node-sdk/src/index.ts
- https://github.com/screenpipe/screenpipe/blob/main/packages/screenpipe-js/common/Operator.ts
