// NOTE:
// In order to run this script on Windows, you must have the VCPKG_ROOT environment variable
// set to the path of the directory containing the vcpkg executable.
// TODO:
...
// - If feasible, install VCPKG packages from upstream sources so the user doesn't have to set VCPKG_ROOT
// - Pin VCPKG packages to a specific version to avoid breaking changes
...
if (platform == "windows") {
	...
	await $`SystemDrive=${process.env.SYSTEMDRIVE} SystemRoot=${process.env.SYSTEMROOT} windir=${process.env.WINDIR} ${process.env.VCPKG_ROOT}\\vcpkg.exe install ${config.windows.vcpkgPackages}`;
}
